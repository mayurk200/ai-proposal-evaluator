...
function NumberedList({ items }: { items: { title: string; detail: string }[] }) {
  return (
...
/* ------------------------------------------------------------------ */
/* Sections — one entry drives the card, the shortcuts sidebar, the    */
/* mobile chip row, and the search filter.                             */
/* ------------------------------------------------------------------ */
...
const sections: Section[] = [
  ...
  },
  {
    ...
    render: () => (
      <>
        ...
        />
        <NumberedList items={pipelineSteps} />
      </>
    ),
  },
  {
    id: 'proposal-id',
    label: 'How the proposal ID works',
    icon: Fingerprint,
    render: () => (
      <>
        <SectionHeader
  ...
  {
    id: 'deletion',
    label: 'Deleting & re-scoring',
    icon: Trash2,
    render: () => (
      <>
        <SectionHeader icon={Trash2} title="Deleting & re-scoring" />
...
export default function AboutPage() {
  ...
  const isVisible = (id: string) => matchIds === null || matchIds.has(id);
  const visibleCount = matchIds === null ? sections.length : matchIds.size;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      for (const s of sections) {
        ...
        const text = `${s.label} ${el?.textContent ?? ''}`.toLowerCase();
  ...
  // Shortcut highlighting: track which visible section is under the top of the viewport.
  useEffect(() => {
    const onScroll = () => {
      let current = '';
      for (const s of sections) {
        if (!isVisible(s.id)) continue;
        const el = document.getElementById(s.id);
        if (el && el.getBoundingClientRect().top <= 130) current = s.id;
      }
      if (!current) {
        current = sections.find((s) => isVisible(s.id))?.id ?? sections[0].id;
      }
      setActiveId(current);
    };
        ...
        <div className="flex-1 min-w-0 space-y-6 pb-24">
          ...
          <div>
            <div className="relative">
              ...
              <input
                ...
                placeholder='Search this page — try "uuid", "ocr", "sha-256", "leaderboard"…'
                aria-label="Search this page"
                className="w-full pl-10 pr-20 py-2.5 rounded-xl bg-surface border border-border text-sm text-text placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-primary/25 focus:border-primary/40 transition-shadow"
              />
              {query && (
                <button
                  onClick={() => setQuery('')}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-medium text-text-muted hover:text-text px-2 py-1 rounded-md hover:bg-accent-light transition-colors"
                >
                  Clear
                </button>
              )}
            ...
            {matchIds !== null && (
              <p className="text-xs text-text-muted mt-2">
                {visibleCount} of {sections.length} sections match “{query.trim()}”
              </p>
            )}
          </div>
          ...
          {matchIds !== null && matchIds.size === 0 && (
            <Card hover={false} className="text-center py-10">
              <p className="text-sm font-medium text-text">Nothing matches “{query.trim()}”</p>
              <p className="text-xs text-text-muted mt-1">
                Try a different term, or{' '}
                <button onClick={() => setQuery('')} className="text-primary font-medium hover:underline">
                  clear the search
                </button>{' '}
                to see all {sections.length} sections.
              </p>
            </Card>
          )}
          ...
          {/* Sections */}
          {sections.map((s) => (
            <section key={s.id} id={s.id} className={cn('scroll-mt-6', !isVisible(s.id) && 'hidden')}>
              <Card hover={false}>{s.render()}</Card>
            </section>
          ))}
        </div>
